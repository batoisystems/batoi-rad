...
$entityId = (int)($this->runData['data']['user']['id'] ?? 0);
$entityUid = htmlspecialchars($this->runData['data']['user']['uid'] ?? '');
?>
<form action="<?php print $this->runData['route']['url'];?>" method="post" onsubmit="return validateForm();">
    ...
    <div class="alert alert-secondary d-flex align-items-center gap-3">
        ...
        <div>
            <div class="fw-semibold mb-0">Editing User</div>
            <div class="text-muted small">ID: <?php echo $entityId; ?> · UID: <?php echo $entityUid; ?></div>
        </div>
    </div>
    ...
    <div class="d-flex flex-wrap gap-2">
        ...
        <a class="btn btn-outline-secondary" href="<?php echo $this->runData['route']['rad_admin_url']; ?>/user/viewone/<?php echo $entityUid; ?>">
            Back to user
        </a>
        <a class="btn btn-outline-secondary" href="<?php echo $this->runData['route']['rad_admin_url']; ?>/user/view">
            Back to users
        </a>
    </div>
</form>
...
<script>
    ...
    function validateIPs() {
        ...
        const ips = ipsField.value.split(',').map(ip => ip.trim()).filter(ip => ip !== '');

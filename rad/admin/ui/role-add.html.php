...
<form action="<?php print $formSubmissionUrl;?>" method="post" class="needs-validation" novalidate>
    <!-- Role Name -->
    <?php
    // if there is post data, then use that to populate the form
    $roleName = '';
    if(isset($this->runData['data']['role']['s_role_name'])) {
        $roleName = $this->runData['data']['role']['s_role_name'];
    }
    ?>
    <?php $routeOptions = $this->runData['data']['route_options'] ?? []; ?>
    <div class="form-group">
        <label for="s_role_name">Role Name <span class="text-danger">*</span></label>
        <input type="text" class="form-control" name="s_role_name" id="s_role_name" value="<?php print $roleName;?>" aria-required="true" required autocomplete="nope">
   ...
   <!-- Is SaaS -->
    <?php
    $scope = $this->runData['data']['role']['s_scope'] ?? 'platform';
    ?>
    ...
    <div class="form-group">
        ...
        <select class="form-control" name="s_scope" id="s_scope">
            <option value="platform" <?php echo ($scope ?? 'platform') === 'platform' ? 'selected': '';?>>Platform (Non-SaaS)</option>
    ...
    <?php $showDefaultRoute = ($scope ?? 'platform') === 'platform'; ?>

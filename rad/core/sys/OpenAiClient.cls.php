...
class OpenAiClient implements AiProviderInterface {
    ...
    public function __construct(array $config, ?ErrorHandler $errorHandler = null) {
        ...
        $this->chatEndpoint = $config['endpoint'] ?? 'https://api.openai.com/v1/chat/completions';
        $this->imageEndpoint = $config['image_endpoint'] ?? 'https://api.openai.com/v1/images/generations';
        $this->embeddingsEndpoint = $config['embeddings_endpoint'] ?? 'https://api.openai.com/v1/embeddings';
        $this->audioTranscribeEndpoint = $config['audio_transcribe_endpoint'] ?? 'https://api.openai.com/v1/audio/transcriptions';
        $this->audioSpeechEndpoint = $config['audio_speech_endpoint'] ?? 'https://api.openai.com/v1/audio/speech';
    ...
    private function postBinary(string $url, array $payload): string {
        $response = $this->sendRequest('json', $url, $payload, true);
        return $response;
    }
